...
> 版本：v1.0 ｜ 日期：2026-08-26 ｜ 状态：已确认
> 决策：D-1/D-2/D-3 技术选型 + 前后端分离式架构（方案 A：WPF 客户端 + 本地后端服务）
...
| D-1 桌面框架 | WPF + .NET Framework 4.8 + HandyControl | 兼容 Win7 SP1/10/11，界面美观 |
| D-2 数据库 | SQLite（System.Data.SQLite + Dapper） | 单文件、零配置、事务支持 |
| D-3 报表 | ClosedXML（Excel）+ PDFsharp（PDF）+ WPF 打印（收据） | 已确认双格式导出 |
| 目标环境 | Win7 SP1 / Win10 / Win11，单机 | 单机部署，无需网络 |
...
4. WPF 客户端与后端均基于 .NET Framework 4.8，Win10 1903+ / Win11 自带运行时；
5. Win7 已停止安全更新（2020-01 起），建议 Win7 机器离线使用，逐步升级 Win10/11。
