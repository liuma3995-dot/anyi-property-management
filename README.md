...
当前版本 **1.1.1** ｜ 平台 **Windows 7 SP1 及以上** ｜ 运行时 **.NET Framework 4.8** ｜ 许可证 **MIT**
...
| 财务收费 | 收费项目维护、账单工作台（草稿→发布→失败重推）、收款登记（部分缴/超额转预存/收据打印）、退款减免调整、支出登记、欠费台账、财务报表、收支明细流水 |
...
> **关于初始口令**：默认管理员账号为 `admin`，初始口令写在 `PropertyManagement.Server\App.config` 的 `DefaultAdminPassword` 中，**仅供本机首次登录使用**。系统会在首次登录时强制修改密码，且所有数据都保存在本机、不对外提供服务，请在任何共享或对外环境部署前先完成改密。
